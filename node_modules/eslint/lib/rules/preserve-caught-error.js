	...
	create(context) {
		...
		function insertCauseIntoOptions(fixer, optionsNode, caughtErrorName) {
			...
			if (properties.length === 0) {
				// Insert inside empty braces: `{}` â†’ `{ cause: err }`
							...
							suggest: [
								{
									...
									fix(fixer) {
										...
										if (errorType === "AggregateError") {
											...
											if (!errorsArg) {
												// Case: `throw new AggregateError()` â†’ insert all arguments
												const lastToken =
											...
											if (!messageArg) {
												// Case: `throw new AggregateError([])` â†’ insert message and options
												return fixer.insertTextAfter(
													errorsArg,
													`, "", { cause: ${caughtError.name} }`,
												);
											...
											if (!optionsArg) {
												// Case: `throw new AggregateError([], "")` â†’ insert error options only
												return fixer.insertTextAfter(
													messageArg,
													`, { cause: ${caughtError.name} }`,
												);
											...
											// Complex dynamic options â€” skip
											return null;
										}
										...
										if (!messageArg) {
											// Case: `throw new Error()` â†’ insert both message and options
											const lastToken =
												sourceCode.getLastToken(
													throwExpression,
												);
										...
										if (!optionsArg) {
											// Case: `throw new Error("Some message")` â†’ insert only options
											return fixer.insertTextAfter(
												messageArg,
												`, { cause: ${caughtError.name} }`,
											);
										...
										return null; // Identifier or spread â€” do not fix
									},
								},
							],
						});
