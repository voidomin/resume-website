...
`,n);return e}function Kr(t){let e,n;return typeof t=="string"?(e=Wr(t),n=t):(Array.isArray(t)&&(t=t[0]),t&&t.context&&(t.lineStarts||(t.lineStarts=Wr(t.context.src)),e=t.lineStarts,n=t.context.src)),{lineStarts:e,src:n}}function Sn(t,e){if(typeof t!="number"||t<0)return null;let{lineStarts:n,src:r}=Kr(e);if(!n||!r||t>r.length)return null;for(let i=0;i<n.length;++i){let o=n[i];if(t<o)return{line:i,col:t-n[i-1]+1};if(t===o)return{line:i+1,col:1}}let s=n.length;return{line:s,col:t-n[s-1]+1}}function ho(t,e){let{lineStarts:n,src:r}=Kr(e);if(!n||!(t>=1)||t>n.length)return null;let s=n[t-1],i=n[t];for(;i&&i>s&&r[i-1]===`
`;)--i;return r.slice(s,i)}function go({start:t,end:e},n,r=80){let s=ho(t.line,n);if(!s)return null;let{col:i}=t;if(s.length>r)if(i<=r-10)s=s.substr(0,r-1)+"\u2026";else{let f=Math.round(r/2);s.length>i+f&&(s=s.substr(0,i+f-1)+"\u2026"),i-=s.length-r,s="\u2026"+s.substr(1-r)}let o=1,a="";e&&(e.line===t.line&&i+(e.col-t.col)<=r+1?o=e.col-t.col:(o=Math.min(s.length+1,r)-i,a="\u2026"));let c=i>1?" ".repeat(i-1):"",l="^".repeat(o);return`${s}
...
`):null}commentHasRequiredWhitespace(e){let{src:n}=this.context;if(this.header&&e===this.header.end||!this.valueRange)return!1;let{end:r}=this.valueRange;return e!==r||t.atBlank(n,r-1)}get hasComment(){if(this.context){let{src:e}=this.context;for(let n=0;n<this.props.length;++n)if(e[this.props[n].start]===ie.COMMENT)return!0}return!1}get hasProps(){if(this.context){let{src:e}=this.context;for(let n=0;n<this.props.length;++n)if(e[this.props[n].start]!==ie.COMMENT)return!0}return!1}get includesTrailingLines(){return!1}get jsonLike(){return[st.FLOW_MAP,st.FLOW_SEQ,st.QUOTE_DOUBLE,st.QUOTE_SINGLE].indexOf(this.type)!==-1}get rangeAsLinePos(){if(!this.range||!this.context)return;let e=Sn(this.range.start,this.context.root);if(!e)return;let n=Sn(this.range.end,this.context.root);return{start:e,end:n}}get rawValue(){if(!this.valueRange||!this.context)return null;let{start:e,end:n}=this.valueRange;return this.context.src.slice(e,n)}get tag(){for(let e=0;e<this.props.length;++e){let n=this.getPropValue(e,ie.TAG,!1);if(n!=null){if(n[1]==="<")return{verbatim:n.slice(2,-1)};{let[r,s,i]=n.match(/^(.*!)([^!]*)$/);return{handle:s,suffix:i}}}}return null}get valueRangeContainsNewline(){if(!this.valueRange||!this.context)return!1;let{start:e,end:n}=this.valueRange,{src:r}=this.context;for(let s=e;s<n;++s)if(r[s]===`
`)return!0;return!1}parseComment(e){let{src:n}=this.context;if(n[e]===ie.COMMENT){let r=t.endOfLine(n,e+1),s=new xe(e,r);return this.props.push(s),r}return e}setOrigRanges(e,n){return this.range&&(n=this.range.setOrigRange(e,n)),this.valueRange&&this.valueRange.setOrigRange(e,n),this.props.forEach(r=>r.setOrigRange(e,n)),n}toString(){let{context:{src:e},range:n,value:r}=this;if(r!=null)return r;let s=e.slice(n.start,n.end);return t.addStringTerminator(e,n.end,s)}},ge=class extends Error{constructor(e,n,r){if(!r||!(n instanceof oe))throw new Error(`Invalid arguments for new ${e}`);super(),this.name=e,this.message=r,this.source=n}makePretty(){if(!this.source)return;this.nodeType=this.source.type;let e=this.source.context&&this.source.context.root;if(typeof this.offset=="number"){this.range=new xe(this.offset,this.offset+1);let n=e&&Sn(this.offset,e);if(n){let r={line:n.line,col:n.col+1};this.linePos={start:n,end:r}}delete this.offset}else this.range=this.source.range,this.linePos=this.source.rangeAsLinePos;if(this.linePos){let{line:n,col:r}=this.linePos.start;this.message+=` at line ${n}, column ${r}`;let s=e&&go(this.linePos,e);s&&(this.message+=`:

${s}
`)}delete this.source}},wn=class extends ge{constructor(e,n){super("YAMLReferenceError",e,n)}},it=class extends ge{constructor(e,n){super("YAMLSemanticError",e,n)}},bn=class extends ge{constructor(e,n){super("YAMLSyntaxError",e,n)}},Nn=class extends ge{constructor(e,n){super("YAMLWarning",e,n)}};function yo(t,e,n){return e in t?Object.defineProperty(t,e,{value:n,enumerable:!0,configurable:!0,writable:!0}):t[e]=n,t}var On=class t extends oe{static endOfLine(e,n,r){let s=e[n],i=n;for(;s&&s!==`
...
`&&(i=h+1,f=!0)}let y=An(g);y&&Array.prototype.push.apply(this.items,y)}return c}setOrigRanges(e,n){return n=super.setOrigRanges(e,n),this.items.forEach(r=>{n=r.setOrigRanges(e,n)}),n}toString(){let{context:{src:e},items:n,range:r,value:s}=this;if(s!=null)return s;let i=e.slice(r.start,n[0].range.start)+String(n[0]);for(let o=1;o<n.length;++o){let a=n[o],{atLineStart:c,indent:l}=a.context;if(c)for(let f=0;f<l;++f)i+=" ";i+=String(a)}return u.Node.addStringTerminator(e,r.end,i)}},Ln=class extends u.Node{constructor(){super(u.Type.DIRECTIVE),this.name=null}get parameters(){let e=this.rawValue;return e?e.trim().split(/[ \t]+/):[]}parseName(e){let{src:n}=this.context,r=e,s=n[r];for(;s&&s!==`
`&&s!=="	"&&s!==" ";)s=n[r+=1];return this.name=n.slice(e,r),r}parseParameters(e){let{src:n}=this.context,r=e,s=n[r];for(;s&&s!==`
`&&s!=="#";)s=n[r+=1];return this.valueRange=new u.Range(e,r),r}parse(e,n){this.context=e;let r=this.parseName(n+1);return r=this.parseParameters(r),r=this.parseComment(r),this.range=new u.Range(n,r),r}},Tn=class t extends u.Node{static startCommentOrEndBlankLine(e,n){let r=u.Node.endOfWhiteSpace(e,n),s=e[r];return s==="#"||s===`
...
`)));let n=[],r=0;do{let s=new Tn,i=new In({src:t});r=s.parse(i,r),n.push(s)}while(r<t.length);return n.setOrigRanges=()=>{if(e.length===0)return!1;for(let i=1;i<e.length;++i)e[i]-=i;let s=0;for(let i=0;i<n.length;++i)s=n[i].setOrigRanges(e,s);return e.splice(0,e.length),!0},n.toString=()=>n.join(`...
`),n}jr.parse=So});var De=ne(M=>{"use strict";var p=ce();function wo(t,e,n){return n?`#${n.replace(/[\s\S]^/gm,`$&${e}#`)}
...
`)n===_n&&(d=Hr(t,d)),m=d+c,g=void 0;else{if(C===" "&&y&&y!==" "&&y!==`
`&&y!=="	"){let L=t[d+1];L&&L!==" "&&L!==`
`&&L!=="	"&&(g=d)}if(d>=m)if(g)l.push(g),m=g+c,g=void 0;else if(n===Dt){for(;y===" "||y==="	";)y=C,C=t[d+=1],h=!0;let L=d>P+1?d-2:w-1;if(f[L])return t;l.push(L),f[L]=!0,m=L+c,g=void 0}else h=!0}y=C}if(h&&a&&a(),l.length===0)return t;o&&o();let A=t.slice(0,l[0]);for(let C=0;C<l.length;++C){let L=l[C],R=l[C+1]||t.length;L===0?A=`
${e}${t.slice(0,R)}`:(n===Dt&&f[L]&&(A+=`${t[L]}\\`),A+=`
${e}${t.slice(L+1,R)}`)}return A}var Rn=({indentAtStart:t})=>t?Object.assign({indentAtStart:t},be.fold):be.fold,Ft=t=>/^(%|---|\.\.\.)/m.test(t);function To(t,e,n){if(!e||e<0)return!1;let r=e-n,s=t.length;if(s<=r)return!1;for(let i=0,o=0;i<s;++i)if(t[i]===`
`){if(i-o>r)return!0;if(o=i+1,s-o<=r)return!1}return!0}function Se(t,e){let{implicitKey:n}=e,{jsonEncoding:r,minMultiLineLength:s}=be.doubleQuoted,i=JSON.stringify(t);if(r)return i;let o=e.indent||(Ft(t)?"  ":""),a="",c=0;for(let l=0,f=i[l];f;f=i[++l])if(f===" "&&i[l+1]==="\\"&&i[l+2]==="n"&&(a+=i.slice(c,l)+"\\ ",l+=1,c=l,f="\\"),f==="\\")switch(i[l+1]){case"u":{a+=i.slice(c,l);let m=i.substr(l+2,4);switch(m){case"0000":a+="\\0";break;case"0007":a+="\\a";break;case"000b":a+="\\v";break;case"001b":a+="\\e";break;case"0085":a+="\\N";break;case"00a0":a+="\\_";break;case"2028":a+="\\L";break;case"2029":a+="\\P";break;default:m.substr(0,2)==="00"?a+="\\x"+m.substr(2):a+=i.substr(l,6)}l+=5,c=l+1}break;case"n":if(n||i[l+2]==='"'||i.length<s)l+=1;else{for(a+=i.slice(c,l)+`

`;i[l+2]==="\\"&&i[l+3]==="n"&&i[l+4]!=='"';)a+=`
`,l+=2;a+=o,i[l+2]===" "&&(a+="\\"),l+=1,c=l+1}break;default:l+=1}return a=c?a+i.slice(c):i,n?a:Bt(a,o,Dt,Rn(e))}function es(t,e){if(e.implicitKey){if(/\n/.test(t))return Se(t,e)}else if(/[ \t]\n|\n[ \t]/.test(t))return Se(t,e);let n=e.indent||(Ft(t)?"  ":""),r="'"+t.replace(/'/g,"''").replace(/\n+/g,`$&
${n}`)+"'";return e.implicitKey?r:Bt(r,n,Zr,Rn(e))}function $t({comment:t,type:e,value:n},r,s,i){if(/\n[\t ]+$/.test(n)||/^\s*$/.test(n))return Se(n,r);let o=r.indent||(r.forceBlockIndent||Ft(n)?"  ":""),a=o?"2":"1",c=e===p.Type.BLOCK_FOLDED?!1:e===p.Type.BLOCK_LITERAL?!0:!To(n,be.fold.lineWidth,o.length),l=c?"|":">";if(!n)return l+`
...
`&&n++,n===1&&/\S/u.test(i))return!1;if(n===2)return!0}return!1}function Mt(t){let{node:e}=t;switch(e.type){case"tag":case"anchor":case"comment":return!1}let n=t.stack.length;for(let r=1;r<n;r++){let s=t.stack[r],i=t.stack[r-1];if(Array.isArray(i)&&typeof s=="number"&&s!==i.length-1)return!1}return!0}function kt(t){return ve(t.children)?kt(D(0,t.children,-1)):t}function Ir(t){return t.value.trim()==="prettier-ignore"}function xr(t){let{node:e}=t;if(e.type==="documentBody"){let n=t.parent.head;return I(n)&&Ir(D(0,n.endComments,-1))}return te(e)&&Ir(D(0,e.leadingComments,-1))}function _e(t){return!ve(t.children)&&!Xi(t)}function Xi(t){return te(t)||ae(t)||dn(t)||K(t)||I(t)}function te(t){return ve(t?.leadingComments)}function ae(t){return ve(t?.middleComments)}function dn(t){return t?.indicatorComment}function K(t){return t?.trailingComment}function I(t){return ve(t?.endComments)}function Rr(t){return t?t.split(/(?<!^| ) (?! |$)/u):[]}function Dr(t,e,n){let r=e.split(`
